...
async function scrapData(req, res) {
  try {
    ...
    // console.log("here Data inserted to DB>>", req.body.nifty50Data);
  ...
  try {
    ...
    if (!data[0]) {
      return res
        .status(404)
        .send({
          message: `Stock data is not found for  given Time stamp please wait we are collecting data`,
        });
    }
    covert(data);
    res
      .status(200)
      .json({
        message: `Sheet updated successfully with Date: ${req.headers.date} & Time: ${req.headers.time}`,
      });
  } catch (error) {
  ...
  try {
    ...
    if (!stocksData[0]) {
      return res
        .status(404)
        .send({
          message: `Stock data is not found for "Sheet2" with given Time please wait we are collecting data`,
        });
    }
  ...
  try {
    ...
    if (!stocksData[0]) {
      return res
        .status(404)
        .send({
          message: `Stock data is not found for "Sheet3" with given Time please wait we are collecting data`,
        });
    }
